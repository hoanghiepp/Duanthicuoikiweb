<section class="card">
    <div class="card-header">
        <div>
            <h2><?= e($title) ?></h2>
            <p>Tìm kiếm, lọc, phân trang, thêm, sửa, xóa dữ liệu.</p>
        </div>

        <a class="btn btn-primary"
           href="<?= app_url(['page' => $config['page'], 'action' => 'create']) ?>">
            + Thêm mới
        </a>
    </div>
        ...
        <?php foreach (($config['filters'] ?? []) as $name => $filter): ?>
            ...
            <?php elseif (!empty($filter['source'])): ?>
                ...
                <select name="<?= e($name) ?>">
                    ...
                    <?php foreach ($options[$filter['source']] ?? [] as $op): ?>
                        <option
                            value="<?= $op['id'] ?>"
                            <?= (string)($_GET[$name] ?? '') === (string)$op['id'] ? 'selected' : '' ?>
                        >
                            <?= e($op['label']) ?>
                        </option>
                    <?php endforeach; ?>
                            ...
                            <?php foreach ($config['columns'] as $col => $label): ?>
                                <td>
                                    ...
                                    <?php elseif ($col === 'status' && $config['page'] === 'employees'): ?>
                                        <span class="badge <?= e($row[$col]) ?>">
                                            <?= e(employee_status_label($row[$col])) ?>
                                        </span>

                                    <?php elseif ($col === 'status' && $config['page'] === 'attendance'): ?>
                                        <span class="badge <?= e($row[$col]) ?>">
                                            <?= e(attendance_label($row[$col])) ?>
                                        </span>

                                    <?php elseif ($col === 'role'): ?>
                                        <?= e(role_label($row[$col])) ?>
